...
var login	= Ti.UI.createButton({title:'Log In', color:'white', backgroundImage:'none'}),
	logout	= Ti.UI.createButton({title:'Log Out', color:'white', backgroundImage:'none', bottom:5, left:5});
...
var	cameraWin	= Ti.UI.createWindow({title:'Scan Devices', backgroundColor:'#484850', barColor:'#B50D00', tabBarHidden:true}),
	deviceWin	= Ti.UI.createWindow({title:'List Of Devices', backgroundColor:'#484850', barColor:'#B50D00', tabBarHidden:true}),
...
cameraWin.addEventListener('swipe', function(e) { if (e.direction == 'left') { deviceWin.open(); /*if(admin == true) deviceWin.setRightNavButton(add);*/ }});
...
deviceWin.addEventListener('swipe', function(e) { if (e.direction == 'right') { deviceWin.close(); /*if(admin == true) deviceWin.setRightNavButton(blank);*/ }});
login.addEventListener("click", function() { userLog.logIn(); });
